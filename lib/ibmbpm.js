...
var getVisualModel = function ( request, callback ) {

    var username = 'bpmadmin';
    var password = 'bpmadmin';
    var auth = 'Basic ' + new Buffer(username + ':' + password).toString('base64');
    var snapshot_id = request.query.snapshot_id;
    var branch_id = request.query.branch_id;
    var po_id = request.query.po_id;

    console.log( '/rest/bpm/wle/v1/visual/processModel/'+ po_id +'?snapshotId='+snapshot_id+'&branchId='+branch_id );

    var options = {
        host: '172.20.12.42',
        port: '9080',
        path: '/rest/bpm/wle/v1/visual/processModel/'+ po_id +'?snapshotId='+snapshot_id+'&branchId='+branch_id,
        headers: {
            Authorization: auth
        }
    };
...
exports.getVisualModel = getVisualModel;
